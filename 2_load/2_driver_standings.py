...
# This notebook is for calculating drivers standings over year based on a logic
...
agg_df = df.groupBy("race_year", "driver_nationality", "driver_name", "team") \
    .agg(sum("points").alias("total_points"),
         count(when(col("position") == 1, True)).alias("wins"))

# COMMAND ----------

#driver standings creation by using rank
...
final_df = agg_df.withColumn("rank", rank().over(driver_rank_spec))
...
overwritePartition(final_df, "f1_presentation", "driver_standings", "race_year" )
